...
class Bot(Chrome):
    ...
    '''
    All elements types referenced in the help functions are \
selenium.webdriver.remote.webelement.WebElement
    ...
    def __enter__(self):
        ...
        Makes the browser go to http://web.whatsapp.com and \
wait for the page to load
    ...
    def wait_for_page(self):
        '''
        This function is called when entering the with statement \
right after http://web.whatsapp.com is entered.
        It exits only when the QR code and the loading screen \
are not on the page.
        It also refreshed the page if the connection is lost.

        The intervals between checks are 500ms, which should be \
adequate considering how slow the whatsapp web servers are.
        '''
        # print("waitando")
    ...
    def copy_to_cipboard(text):
        '''
        This function copies text to the clipboard in order to make \
writing text easier and faster
        '''
        pyperclip.copy(text)
    ...
    def send_message_element(self, element, text):
        '''
        Copies the text to the clipboard then writes it to an element /
and hits the enter key
    ...
    def write_message_keys_element(element, text):
        ...
        Note: this the slowest function to write to an element and /
should only be used if the others fail
        '''
        element.send_keys(text)

    def send_message_keys_element(self, element, text):
        '''
        Writes text to an element by simulating keystrokes and hits /
the enter key
