...
if os.name == "posix":
    try:
        b = webdriver.Chrome("./chromedriver_linux", options=options)
    except WebDriverException:

        b = webdriver.Chrome("./chromedriver_mac", options=options)
else:
        ...
        for i in range(len(elem)):
            ...
            if len(possiveis_nomes) == 1:
                ...
                try:
                    for j, i in enumerate(possiveis_nomes):
                        print(f'{j+1} - {i.text}')
                    name_index = int(input("Digite o número da pessoa/grupo"))
                    name_index += 1
